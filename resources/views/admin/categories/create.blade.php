...
@section('title', '카테고리 생성')
@section('header', '새 카테고리 생성')
@section('description', '새로운 카테고리를 생성합니다')
...
<div id="category-create-app">
    <div class="max-w-2xl">
        ...
        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('admin.categories.store') }}" method="POST" @submit="handleSubmit">
                @csrf

                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">카테고리 정보</h3>
                    <p class="mt-1 text-sm text-gray-500">카테고리의 기본 정보를 입력해주세요.</p>
                </div>
                ...
                <div class="px-6 py-6 space-y-6">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">부모 카테고리를 선택하면 하위 카테고리로 생성됩니다.</p>
                        @error('parent_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
