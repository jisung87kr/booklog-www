...
const saveBookStore = async () => {
    try {
        if(bookcase.value.id) {
            ...
            return sendRequest('PUT', `/api/users/${auth.value.username}/bookcases/${bookcase.value.id}`, params);
        } else {
            let params = {
                title: bookcase.value.title,
                description: bookcase.value.description,
            ...
            return sendRequest('POST', `/api/users/${auth.value.username}/bookcases`, params);
        }
    } catch (error) {
        console.error(error);
    }
...
<template>
    <Transition name="slide-fade">
        <div v-show="loaded" class="min-h-screen bg-gray-50">
            <!-- Modern header -->
            <header-component class="sticky top-0 z-30 bg-white border-b border-gray-200">
